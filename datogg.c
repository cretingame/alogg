...
static float find_quality(AL_CONST DATAFILE *dat)
{
   if (dat && dat->prop) {
      AL_CONST char *value=get_datafile_property(dat,DAT_OGG_VORBIS_QUALITY);
      if (value && *value) {
         int n=atoi(value);
         n=MID(0,n,100);
         return n/100.0f;
...
#if 0//ALLEGRO_VERSION*0x10000+ALLEGRO_SUB_VERSION*0x100+ALLEGRO_WIP_VERSION>=0x040109
static DATAFILE *grab_ogg(AL_CONST char *filename, AL_CONST DATAFILE *prev, int depth)
#else
...
#endif
{
   ...
   if (extension && !stricmp(extension,"ogg")) {
     ...
     pack_fclose(f);
#if 0//ALLEGRO_VERSION*0x10000+ALLEGRO_SUB_VERSION*0x100+ALLEGRO_WIP_VERSION>=0x040109
     return datedit_wrap(ovd, DAT_OGG_VORBIS, 0);
#else
     return ovd;
#endif
   }
...
#if 0//ALLEGRO_VERSION*0x10000+ALLEGRO_SUB_VERSION*0x100+ALLEGRO_WIP_VERSION>=0x040109
         DATAFILE *dat;
         float quality=find_quality(prev);
         char qstr[8];
         ovd=encode_sample(sample,quality);
         destroy_sample(sample);
         dat=datedit_wrap(ovd,DAT_OGG_VORBIS,0);
         if (!dat) return NULL;
...
static int encode_objects_worker(DATAFILE *dat, int *param, int param2)
{
   ...
   if (dat->type == DAT_SAMPLE) {
      float quality=find_quality(dat);
